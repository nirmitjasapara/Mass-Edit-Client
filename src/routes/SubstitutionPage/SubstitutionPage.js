...
export default class SubstitutionPage extends Component {
  ...
  state = { error: null, selection: "", substitutes: [{ from: "", to: "" }] };
  ...
  renderOptions() {
    ...
    return (
      ...
      >
        {templates.map((x, i) => (
          <option key={"template_" + i} value={x.id}>
            {x.name}
          </option>
        ))}
        <option key="template_default" value={""}>
          Create template
        </option>
      </select>
    );
    ...
    return (
      <main className="main">
        <Section className="AddPage">
          <h2>Substitution</h2>
          <p>
            Please add two words to substitute one with the other. You may also
            click the plus button to add more replacements, or the minus (for 2+
            substitutions) to delete one.
          </p>
          <p className="text-box">
            {this.context.getOriginal(Number(this.props.match.params.id))?.text}
          </p>
          <form className="SubstitutionForm" onSubmit={this.handleSubmit}>
            <div className="options">{this.renderOptions()}</div>
            {this.state.selection ? "" : this.renderSubstitutes()}
